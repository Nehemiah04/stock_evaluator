...
def load_institution_universe(file_path: str = str(UNIVERSE_PATH)) -> pd.DataFrame:
    ...
    df["trackability_score"] = df["trackability"].map(
        {
            "Low": -1,
            "Medium": 0,
            "High": 1
...
def build_institution_summary(df: pd.DataFrame) -> dict:
    ...
    high_trackability_count = len(
        df[df["trackability"].str.lower() == "high"]
    )

    bank_count = len(
        df[df["type"].str.lower() == "bank"]
    )
    ...
    hedge_fund_count = len(
        df[df["type"].str.lower().str.contains("hedge")]
    )
